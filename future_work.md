...
- **Parallelism.** We should aim for massive parallelization across agent interactions. This includes better modularization of agents so they can be orchestrated more efficiently. Consider asyncio.
...
- **Publish to PyPI.** We want to push this specific agent library to PyPI so it can be dropped into any mini data platform as a reusable package.
...
- **Response Feedback.** Users should be able to rate a response which contributes back to the system's development. Consider a semantic index.

## Data Platform

- **Semantic index over metadata.** With enough data, we want to introduce a semantic index that maps the metadata both explicitly declared and implicitly inferred from the data CSVs. This would enable richer discovery and more intelligent query planning. Also relevant for human feedback on ai responses.
...
- **Scale-aware technology choices.** We acknowledge that the scale of this project is constrained. At massive scale — the kind Astronomer actually deals with — there are additional optimization and technology choices that become relevant. For example:
  - Intelligent DFS (depth-first search) for an agent to make sense of data using a semantic layer. Benefits from parallelization and 
...
- Some agent evaluations are flapping a bit and need to be more thoroughly ironed out. But alas it is sunday night haha.
